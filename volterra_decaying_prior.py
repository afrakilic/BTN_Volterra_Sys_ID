...
class btnkm:
    ...
    def train(
        ...
        g0_m = shape_parameter_lambda_M
            
        g_N = g0_m
        h0_m = scale_parameter_lambda_M

        h_N = h0_m
        g_array = np.array(g0_m)  # shape: (order, memory)
        h_array = np.array(h0_m)  # shape: (order, memory)  
        lambda_M = (h_array / g_array).tolist() 
        ...
        for it in pbar:
            ...
            if lambda_M_update:
                for d in range(D):
                    ...
                    g_N[d] = np.array(g0_m[d]) + R / 2
                    h_N[d] = np.array(h0_m[d]) * np.ones(I) + (mtemp + vtemp) / 2
            ...
            temp5 = sum(
                np.sum(
                    ...
                    + np.array(g_N[d])
                    * (1 - safelog(np.array(h_N[d])) - (np.array(h0_m[d]) / np.array(h_N[d])))
                )
